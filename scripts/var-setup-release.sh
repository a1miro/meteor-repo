...
echo "DL_DIR = \"/opt/downloads\"" >> conf/local.conf
echo "SSTATE_DIR = \"/opt/sstate-cache\"" >> conf/local.conf
echo "CONNECTIVITY_CHECK_URIS=\"https://google.com\"" >> conf/local.conf
...
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-wheelme\""     >> $BUILD_DIR/conf/bblayers.conf
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-wheelme-dev\"" >> $BUILD_DIR/conf/bblayers.conf
